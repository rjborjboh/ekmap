...
(function($){
    $.extend({
        handleError : function(s, xhr, status, e) {
            ...
            if (s.global) {
                (s.context ? $(s.context) : $.event).trigger(
                        "ajaxError", [ xhr, s, e ]);
            }
        },
        ...
        createUploadForm : function(id, fileElementIds) {
            ...
            if (typeof (fileElementIds) == 'string') {
                var tmpStr = fileElementIds.split(',');
                fileElementIds = tmpStr;
            }
            for ( var index in fileElementIds) {
                var oldElement = $('#' + fileElementIds[index]);
                var newElement = $(oldElement).clone();
                $(oldElement).attr('id', fileId);
                // set file attr[name] constant value.
                $(oldElement).attr('name', 'file');
                $(oldElement).before(newElement);
                $(oldElement).appendTo(form);
            }
            // set auth-token info
            var token = $('<input type="hidden" name="ekMap_token" value="tokenStart'
                            + commonjs.storage("Auth-Token")
                            +'tokenEnd">');
            $(token).appendTo(form);
        ...
        ajaxFileUpload : function(s) {
            ...
            var form = $.createUploadForm(id, s.fileElementIds);
            if (s.data)
