...
(function($, undefined) {
	...
	// 自定义提示隐藏、绑定事件以及解绑事件
	$.remindLayer = (function() {
		var winWidth = $(window).width();
		var fnMouseDown = function(e) {
			if (!e || !e.target) return;
			if (e.target.id !== $.remindLayer.id && $(e.target).parents("#" + $.remindLayer.id).length === 0) {
				$.remindLayer.hide();
			}	
			...
			isOverflow: function(ele) {
				...
				if (!attrMin && !attrMax) {
					...
					if (attrDataMin && value.length < attrDataMin) {
						$(ele).remindLayer("至少输入" + attrDataMin + "个字符");
						ele.focus();
					} else if (attrDataMax && value.length > attrDataMax) {
						$(ele).remindLayer("最多输入" + attrDataMax + "个字符");
					...
					if (attrMin && value < attrMin) {
						$(ele).remindLayer("值必须大于或等于" + attrMin);	
					} else if (attrMax && value > attrMax) {
						$(ele).remindLayer("值必须小于或等于" + attrMax);	
				  ...
				  , remind = function(control, type, tag) {
					...
					if ($(control).isVisible()) {
						if (type == "radio" || type == "checkbox") {
							$(control).remindLayer(OBJREG.prompt[type], {
								align: "left"	
							});
							control.focus();
						} else if (tag == "select" || tag == "empty") {
							// 下拉值为空或文本框文本域等为空
							$(control).remindLayer((tag == "empty" && text)? "您尚未输入"+ text : OBJREG.prompt[tag]);
							control.focus();
						} else if (/^range|number$/i.test(type) && Number(control.value)) {
							// 整数值与数值的特殊提示
							$(control).remindLayer("值无效");
							control.focus();
							control.select();
						} else {
							// 文本框文本域格式不准确
							...
							var errMsg = $(control).attr('data-invalidmsg');
							if (text) {
							...
							if (errMsg) finalText += '</br>' + commonjs.renderMsg(errMsg);
							$(control).remindLayer(finalText);
							control.focus();
							control.select();	
						}			
					} else {
	...
	$.fn.extend({
		...
		remindLayer: function(content, options) {
			var defaults = {
				size: 6,	// 三角的尺寸
			...
			options = options || {};
			options.css = $.extend({}, defaults.css, options.css || $.remindLayer.css);
			...
			if ($("#" + $.remindLayer.id).size()) $.remindLayer.hide();
			
			this.remind = $('<div id="'+ $.remindLayer.id +'">'+ content +'</div>').css(params.css);
			...
			$.remindLayer.display = true;
			
			// 绑定消除事件
			$.remindLayer.target = $(this).addClass("error");
			$.remindLayer.bind();
